...
const STORAGE_KEY = 'snapflix_session';
...
export interface SnapflixSession {
  msisdn: string;
  isLoggedIn: boolean;
  subscription: StoredSubscription | null;
}

interface SnapflixStorage {
  session: SnapflixSession | null;
  subscriptionsByMsisdn: Record<string, StoredSubscription>;
}

const emptyStorage = (): SnapflixStorage => ({
  session: null,
  subscriptionsByMsisdn: {},
});

const readStorage = (): SnapflixStorage => {
  try {
    ...
    const parsed = JSON.parse(raw) as SnapflixStorage;
    return {
...
const writeStorage = (storage: SnapflixStorage): void => {
